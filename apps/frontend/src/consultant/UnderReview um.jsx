// UnderReview_um.jsx — Utility Model
...
export default function UnderReviewum() {
    ...
    const load = useCallback(async () => {
        try {
            const res = await axios.get(`${API}/umid-submissions-under-review`, { headers: hdrs() });
            const data = Array.isArray(res.data) ? res.data : (res.data?.data || []);
            setSubmissions(data.filter(s => !s.ip_type || s.ip_type === 'Utility Model'));
            setError(null);
        } catch (err) {
            console.error('❌', err);
    ...
    const handleSave = async (checklist, notes) => {
        await axios.put(`${API}/umid-checklist-update/${selected.id}`,
    ...
    const handleApprove = async (notes) => {
        await axios.put(`${API}/umid-review-action/${selected.id}`,
            ...
            <div className="ur-table-card">
                {submissions.length === 0 ? (
                    <div className="ur-empty">
                        ...
                        <p>No Utility Model submissions are currently under review.</p>
                    </div>
                ) : (
                    ...
                    <table className="ur-table">
                        ...
                        <tbody>
                            {submissions.map(s => {
                                return (
                                    <tr key={s.id}>
                                        <td><span className="ur-ref-badge">UM-{s.id}</span></td>
                                        ...
                                        <td style={{ fontFamily: 'var(--mono)', fontSize: 12 }}>{fmt(s.assigned_at)}</td>
                                        <td>
