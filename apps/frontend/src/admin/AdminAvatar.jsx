...
import ProfileImg from './Images/Flor_Vega.jpg';
...
function AdminAvatar() {
    ...
    const [userType,        setUserType]        = useState(stored.userType || 'Admin');
    ...
    useEffect(() => {
        ...
        // Listen for photo changes fired by AdminProfile — update avatar photo instantly
        const onPhotoUpdate = (e) => {
            const { filename } = e.detail;
            setPhotoUrl(filename ? buildPicUrl(filename) : null);
        };

        // Listen for name/detail changes fired by AdminProfile — update avatar name instantly
        const onProfileUpdate = (e) => {
            const { fullName: updatedName } = e.detail;
        ...
        return () => {
            window.removeEventListener('profile-picture-updated', onPhotoUpdate);
